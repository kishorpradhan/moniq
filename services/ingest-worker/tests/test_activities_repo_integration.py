...
def test_batch_upsert_updates_existing_row():
    ...
    try:
        with conn:
            ...
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT id, quantity, price, amount, activity_date FROM activities WHERE user_id = %s AND external_transaction_id = %s",
                    (user_id, str(activity_id)),
                )
                row = cur.fetchone()
                assert row is not None
                assert str(row[0]) == str(activity_id)
                assert str(row[1]) == "2.00"
                assert str(row[2]) == "12.00"
                assert str(row[3]) == "24.00"
                assert str(row[4]) == "2026-02-05"
    finally:
        with conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM activities WHERE user_id = %s AND external_transaction_id = %s",
                    (user_id, str(activity_id)),
                )
        conn.close()
